...
class IPIPANCorpusReader(CorpusReader):
    ...
    The corpus includes information about text domain, channel and download.
    You can access possible values using ``domains()``, ``channels()`` and
    ``download()``. You can use also this metadata to filter files, e.g.:
    ``fileids(channel='prasa')``, ``fileids(download='publicystyczny')``.
    ...
    def fileids(self, channels=None, domains=None, categories=None):
        if channels is not None and domains is not None and categories is not None:
            raise ValueError(
                ...
                "and download parameter at once"
            )
