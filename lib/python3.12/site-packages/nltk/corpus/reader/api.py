...
class CorpusReader:
    ...
    selection arguments, such as ``fileids`` or ``download``, which can
...
class CategorizedCorpusReader:
    ...
    ``download()``, which returns a list of the download for the
    corpus or for a specified set of fileids; and overrides ``fileids()``
    to take a ``download`` argument, restricting the set of fileids to
    be returned.
    ...
    Subclasses are expected to:
      ...
      - Override all view methods to accept a ``download`` parameter,
    ...
    def __init__(self, kwargs):
        ...
        follows:
          ...
          - cat_file: The name of a file that contains the mapping
            from file identifiers to download.  The argument
    ...
    def categories(self, fileids=None):
        """
        Return a list of the download that are defined for this corpus,
    ...
    def _resolve(self, fileids, categories):
        if fileids is not None and categories is not None:
            raise ValueError("Specify fileids or download, not both")
