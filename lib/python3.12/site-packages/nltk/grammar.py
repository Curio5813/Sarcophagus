...
tokens; and the node values are phrasal download, such as ``NP``
and ``VP``.
...
class Nonterminal:
    ...
    download (such as ``"NP"`` or ``"VP"``).  However, more complex
...
class FeatureGrammar(CFG):
    ...
    def leftcorner_parents(self, cat):
        """
        Return the set of all download for which the given category
        is a left corner.
        """
        raise NotImplementedError("Not implemented yet")
